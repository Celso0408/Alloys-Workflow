...
In this workflow, the SimStack framework is used to perform the generation of configurations to analyze the thermodynamic properties of binary alloys within the Generalized Quasichemical Approximation (GQCA). For this, five different **WaNos** are combined: SOD-2022, Mult-It, UnpackMol, DFT-VASP and Table-Generator. A table containing the clusters' total energies, degeneracy factors, and number of site substitutions is the expected output of this protocol.
 
To make the Workflow depicted in **Figure 1**, you need to use the drag-and-drop standard procedure of Simstack in five steps. In the first, SOD-2022 **Wano** is used to generate substituted configurations and their respective degeneracy factors. In the second step, we added the Mult-It **Wano** below of SOD-2022 to create a tar file of all structures generated by SOD-2022. In the third step, we add the Unpackmol **Wano** inside AdvancedFor loop control to generate the POSCAR files of substituted configurations of the alloy clusters. In the fourth step, we insert the DFT-VASP **Wano** to receive the POSCAR files from the preceding step and execute the DFT calculations. At the end of the protocol, Table-Generator **WaNo** extracts in a yml file the number which indentify a given configuration generated by SOD-2022 with its respective total energy from the OUTCAR file.
 
 ### This workflow allows us able to:
```
1. From a given supercell expansion, define the number of substitutions to be made for a given atomic kind and generate their degeneracy factors (SOD-2022).
...
**Figure 1** _This workflow aims to perform several DFT calculations of cluster configurations within GQCA framework. It is made from SOD-2022, Mult-It, UnpackMol, DFT-VASP, and Table-Generator **WaNos** connected by the AdvancedFor loop control. In step 1, we generate the configurations containing a given number of atomic substitutions, one atomic specie is substituted by other and their degeneracy factors are calculated by using space group symmetry operations from the pristine crystal structure. Steps 2, 3 and 4 define group the supercell configurations and set the DFT calculation methods employed in the simulation. In the last step, Table-Generator **Wano** extracts the required variables of the output files from the previous steps._
